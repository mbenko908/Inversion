...
;    data_together
     perpath='/mnt/RAIDofsH/mbenko/data1/inversion/calcium/data_processing/' ; adresar kde sa nachadzju velke textove subory obsahyjuce vsetky hodnoty
restore,'mask1.save'
...
;   cin =  transpose(cin, [1,0])
   sirres =  transpose(sirres, [1,0,2])
   sirerr = transpose(sirerr, [1,0,2])

save,filename='/mnt/RAIDofsH/mbenko/data1/inversion/calcium/data_processing/phys_param_gris_ca_10839_big_map_1_jun_20.save',sirres,sirerr, stokes, cin;,cin;,cint,stokes ; treba menit podla toho kam chces
; ulozit data
;stop
   return 
   end
