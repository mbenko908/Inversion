...
;
    alpha=53.9;65.95;53.9;65.9521  ; theta in degrees
    beta=3.0;9.05;-21.43;-8.89;-9.05;8.7093678
    if keyword_set(TIPSC) then begin
       alpha=53.9;65.9521
       beta=3;3;22;3;-9.05;-8.7093678
    endif
;
...
;
 vecd(*,*,8)=reform(extrc[*,*,4]*cos(!dtor*vecd[*,*,3])) ;Bz
    ...
    vecd(*,*,6)=reform(extrc[*,*,4]*sin(!dtor*vecd[*,*,3])*cos(!dtor*vecd(*,*,4))) ;Bx 
                                ; stop
    tvscl, (cint gt 0.5)*vecd[*,*,8]>0<(2900.)
    tvscl, vecd[*,*,8], 400, 0
;stop
    save,filename='tip.save',extrc,vecd, cint, stokes
stop
    return
    end
